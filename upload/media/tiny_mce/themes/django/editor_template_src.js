...
tinyMCE.importThemeLanguagePack('django');

var TinyMCE_DjangoTheme = {
	...
	getControlHTML : function(button_name) {
		...
		for (i=0; i<TinyMCE_DjangoTheme._buttons.length; i++) {
			but = TinyMCE_DjangoTheme._buttons[i];
		...
		switch (button_name) {
			case "formatselect":
				...
				var formats = tinyMCE.getParam("theme_django_blockformats", "p,address,pre,h1,h2,h3,h4,h5,h6", true).split(',');
				var lookup = [
			...
			case "styleselect":
				return '<select id="{$editor_id}_styleSelect" onmousedown="tinyMCE.themes.django._setupCSSClasses(\'{$editor_id}\');" name="{$editor_id}_styleSelect" onfocus="tinyMCE.addSelectAccessibility(event,this,window);" onchange="tinyMCE.execInstanceCommand(\'{$editor_id}\',\'mceSetCSSClass\',false,this.options[this.selectedIndex].value);" class="mceSelectList">{$style_select_options}</select>';
			...
			case "fontselect":
				...
				var fonts = tinyMCE.getParam("theme_django_fonts", nFonts).split(';');
	...
	execCommand : function(editor_id, element, command, user_interface, value) {
		switch (command) {
			...
			case "mceLink":
				...
				if (inst.settings['insertlink_callback']) {
					...
					if (returnVal && returnVal['href'])
						TinyMCE_DjangoTheme._insertLink(returnVal['href'], returnVal['target'], returnVal['title'], returnVal['onclick'], returnVal['style_class']);
				} else {
			...
			case "mceImage":
				...
				if (inst.settings['insertimage_callback']) {
					...
					if (returnVal && returnVal['src'])
						TinyMCE_DjangoTheme._insertImage(returnVal['src'], returnVal['alt'], returnVal['border'], returnVal['hspace'], returnVal['vspace'], returnVal['width'], returnVal['height'], returnVal['align'], returnVal['title'], returnVal['onmouseover'], returnVal['onmouseout']);
				} else
			...
			case "forecolor":
				...
				TinyMCE_DjangoTheme._hideMenus(editor_id);
				...
				if (!fcp.exists()) {
					...
					tinyMCE.addEvent(elm, 'click', TinyMCE_DjangoTheme._handleMenuEvent);
					tinyMCE.addEvent(elm, 'mouseover', TinyMCE_DjangoTheme._handleMenuEvent);
					tinyMCE.addEvent(elm, 'mouseout', TinyMCE_DjangoTheme._handleMenuEvent);
				}
			...
			case "forecolorMenu":
				...
				if (!ml.exists())
					ml.create('div', 'mceMenu', document.body, TinyMCE_DjangoTheme._getColorHTML(editor_id, 'theme_django_text_colors', 'forecolor'));
			...
			case "HiliteColorMenu":
				...
				if (!ml.exists())
					ml.create('div', 'mceMenu', document.body, TinyMCE_DjangoTheme._getColorHTML(editor_id, 'theme_django_background_colors', 'HiliteColor'));
			...
			case "mceColorPicker":
				if (user_interface) {
					...
					template['width'] += tinyMCE.getLang('lang_theme_django_colorpicker_delta_width', 0);
					template['height'] += tinyMCE.getLang('lang_theme_django_colorpicker_delta_height', 0);
			...
			case "mceCodeEditor":
				...
				template['width'] = parseInt(tinyMCE.getParam("theme_django_source_editor_width", 720));
				template['height'] = parseInt(tinyMCE.getParam("theme_django_source_editor_height", 580));
			...
			case "mceCharMap":
				...
				template['width'] += tinyMCE.getLang('lang_theme_django_charmap_delta_width', 0);
				template['height'] += tinyMCE.getLang('lang_theme_django_charmap_delta_height', 0);
			...
			case "mceInsertAnchor":
				...
				template['width'] += tinyMCE.getLang('lang_theme_django_anchor_delta_width', 0);
				template['height'] += tinyMCE.getLang('lang_theme_django_anchor_delta_height', 0);
	...
	getEditorTemplate : function(settings, editorId) {
		...
		var template = new Array();
		var deltaHeight = 0;
		var resizing = tinyMCE.getParam("theme_django_resizing", false);
		var path = tinyMCE.getParam("theme_django_path", true);
		var statusbarHTML = '<div id="{$editor_id}_path" class="mceStatusbarPathText" style="display: ' + (path ? "block" : "none") + '">&#160;</div><div id="{$editor_id}_resize" class="mceStatusbarResize" style="display: ' + (resizing ? "block" : "none") + '" onmousedown="tinyMCE.themes.django._setResizing(event,\'{$editor_id}\',true);"></div><br style="clear: both" />';
		var layoutManager = tinyMCE.getParam("theme_django_layout_manager", "SimpleLayout");

		// Setup style select options -- MOVED UP FOR EXTERNAL TOOLBAR COMPATABILITY!
		var styleSelectHTML = '<option value="">{$lang_theme_style_select}</option>';
		if (settings['theme_django_styles']) {
			var stylesAr = settings['theme_django_styles'].split(';');
			...
			TinyMCE_DjangoTheme._autoImportCSSClasses = false;
		}

		switch(layoutManager) {
			case "SimpleLayout" : //the default TinyMCE Layout (for backwards compatibility)...
				var toolbarHTML = "";
				var toolbarLocation = tinyMCE.getParam("theme_django_toolbar_location", "bottom");
				var toolbarAlign = tinyMCE.getParam("theme_django_toolbar_align", "center");
				var pathLocation = tinyMCE.getParam("theme_django_path_location", "none"); // Compatiblity
				var statusbarLocation = tinyMCE.getParam("theme_django_statusbar_location", pathLocation);
				var defVals = {
					theme_django_buttons1 : "bold,italic,underline,strikethrough,separator,justifyleft,justifycenter,justifyright,justifyfull,separator,styleselect,formatselect",
					theme_django_buttons2 : "bullist,numlist,separator,outdent,indent,separator,undo,redo,separator,link,unlink,anchor,image,cleanup,help,code",
					theme_django_buttons3 : "hr,removeformat,visualaid,separator,sub,sup,separator,charmap"
				};
				...
				// Render rows
				for (var i=1; i<100; i++) {
					var def = defVals["theme_django_buttons" + i];

					var buttons = tinyMCE.getParam("theme_django_buttons" + i, def == null ? '' : def, true, ',');
					if (buttons.length == 0)
						break;

					buttons = removeFromArray(buttons, tinyMCE.getParam("theme_django_disable", "", true, ','));
					buttons = addToArray(buttons, tinyMCE.getParam("theme_django_buttons" + i + "_add", "", true, ','));
					buttons = addToArray(tinyMCE.getParam("theme_django_buttons" + i + "_add_before", "", true, ','), buttons);
			...
			case "RowLayout" : //Container Layout - containers defined in "theme_django_containers" are rendered from top to bottom.
				template['html'] = '<table class="mceEditor" border="0" cellpadding="0" cellspacing="0" width="{$width}" height="{$height}" style="width:{$width}px;height:{$height}px"><tbody>';

				var containers = tinyMCE.getParam("theme_django_containers", "", true, ",");
				var defaultContainerCSS = tinyMCE.getParam("theme_django_containers_default_class", "container");
				var defaultContainerAlign = tinyMCE.getParam("theme_django_containers_default_align", "center");
				...
				for (var i = 0; i < containers.length; i++)
				{
					...
					else if (containers[i] == "mceElementpath" || containers[i] == "mceStatusbar") // ... mceElementpath:
					{
						...
						var curContainer = tinyMCE.getParam("theme_django_container_"+containers[i], "", true, ',');
						var curContainerHTML = "";
						var curAlign = tinyMCE.getParam("theme_django_container_"+containers[i]+"_align", defaultContainerAlign);
						var curCSS = tinyMCE.getParam("theme_django_container_"+containers[i]+"_class", defaultContainerCSS);

						curContainer = removeFromArray(curContainer, tinyMCE.getParam("theme_django_disable", "", true, ','));
			...
			case "CustomLayout" : //User defined layout callback...
				var customLayout = tinyMCE.getParam("theme_django_custom_layout","");
	...
	initInstance : function(inst) {
		if (tinyMCE.getParam("theme_django_resizing", false)) {
			if (tinyMCE.getParam("theme_django_resizing_use_cookie", true)) {
				var w = TinyMCE_DjangoTheme._getCookie("TinyMCE_" + inst.editorId + "_width");
				var h = TinyMCE_DjangoTheme._getCookie("TinyMCE_" + inst.editorId + "_height");

				TinyMCE_DjangoTheme._resizeTo(inst, w, h, tinyMCE.getParam("theme_django_resize_horizontal", true));
			}
		}
	...
	hideInstance : function(inst) {
		TinyMCE_DjangoTheme._hideMenus(inst.editorId);
	},
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection, setup_content) {
		...
		if (selectElm) {
			TinyMCE_DjangoTheme._setupCSSClasses(editor_id);
	...
	_setupCSSClasses : function(editor_id) {
		...
		if (!TinyMCE_DjangoTheme._autoImportCSSClasses)
			return;
	...
	_resizeEventHandler : function(e) {
		var resizer = TinyMCE_DjangoTheme._resizer;
		...
		switch (e.type) {
			...
			case "mouseup":
				TinyMCE_DjangoTheme._setResizing(e, editorId, false);
				TinyMCE_DjangoTheme._resizeTo(tinyMCE.getInstanceById(editorId), resizer.width + dx, resizer.height + dy, resizer.horizontal);
				...
				if (tinyMCE.getParam("theme_django_resizing_use_cookie", true)) {
					...
					// Set the cookies
					TinyMCE_DjangoTheme._setCookie("TinyMCE_" + editorId + "_width", "" + (resizer.horizontal ? resizer.width + dx : ""), expires);
					TinyMCE_DjangoTheme._setCookie("TinyMCE_" + editorId + "_height", "" + (resizer.height + dy), expires);
				}
				break;
		}
	},
	...
	_setResizing : function(e, editor_id, state) {
		...
		if (state) {
			...
			if (!resizer.eventHandlers) {
				if (tinyMCE.isMSIE)
					tinyMCE.addEvent(document, "mousemove", TinyMCE_DjangoTheme._resizeEventHandler);
				else
					tinyMCE.addEvent(window, "mousemove", TinyMCE_DjangoTheme._resizeEventHandler);

				tinyMCE.addEvent(document, "mouseup", TinyMCE_DjangoTheme._resizeEventHandler);
			...
			resizer.horizontal = tinyMCE.getParam("theme_django_resize_horizontal", true);
		} else {
	...
	_getColorHTML : function(id, n, cm) {
		...
		cl = tinyMCE.getParam(n, TinyMCE_DjangoTheme._defColors).split(',');
		...
		if (tinyMCE.getParam("theme_django_more_colors", true))
			h += '<a href="javascript:void(0);" onclick="TinyMCE_DjangoTheme._pickColor(\'' + id + '\',\'' + cm + '\');" class="mceMoreColors">' + tinyMCE.getLang('lang_more_colors') + '</a>';
...
tinyMCE.addTheme("django", TinyMCE_DjangoTheme);

// Add default buttons maps for django theme and all internal plugins
tinyMCE.addButtonMap(TinyMCE_DjangoTheme._buttonMap);
